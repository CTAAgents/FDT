...
所有模块通过 `unified_logger.py` 输出 `fdb_{date}.log`，格式一致。
...
| 指标 | 类型 | 来源 | 说明 |
|:--|:--|:--|:-----|
| `llm_parse_calls_total` | counter | `parse_llm_output()` | LLM 输出解析总调用次数（v9.22.2 新增） |
| `llm_parse_failures_total` | counter | `parse_llm_output()` | LLM 输出解析失败次数（v9.22.2 新增） |

### v9.22.2 新增：`parse_llm_output` 函数

统一 LLM 输出解析入口，封装 `scripts/enforce_structured_output.py` 的调用，提供兼容回退路径。

- **位置**: `fdt_langgraph/llm_provider.py`
- **签名**: `parse_llm_output(raw: str, agent_name: str = "", default: dict = None) -> dict`
