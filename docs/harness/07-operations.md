...
| 版本 | 日期 | 变更 |
|:-----|:-----|:-----|
| **v0.14.1** | 2026-07-27 | **GAP-015: Wind 数据源集成 Phase 1~3 — 全市场数据增强** — ① Phase 1: `data_adapter/sources/wind_source.py` 新增 `WindSource` 适配器（Wind CLI 封装器，6 个公开方法：get_edb/get_convertible_bond/get_fund_holdings/get_fund_nav/get_announcements/get_financial_news），`data_adapter/__init__.py` 注册 6 个 `get_wind_*()` 接口 ② Phase 2: `state.py` 新增 `wind_data` 字段，`_nodes_prepare.py` 新增 `_collect_wind_data()` 采集函数（宏观 EDB 12h 缓存 + 可转债估值 + ETF 持仓/净值）③ Phase 3: `_nodes_context.py` 新增 `_build_wind_context_block()` 等 4 个格式化函数，注入探源/读心 prompt。新增配置 `FDT_WIND_CACHE_TTL`/`FDT_WIND_CACHE_DIR`。版本号 bump 0.14.0→0.14.1 |
| **v0.14.0** | 2026-07-27 | **品种级优化反馈闭环落地 — symbol_params.json + 校准精度提升** — 新增 `fdt_eval/feedback/_config/symbol_params.json` 品种级交易参数存储；`position_base_pct`/`atr_stop_multiplier`/`confidence_offset` 等参数按品种差异化；校准精度 ECE < 0.05；反馈闭环集成到 evolution_graph。版本号 bump 0.13.0→0.14.0 |
