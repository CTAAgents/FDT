...
> LangGraph StateGraph 声明式图编排，现为唯一执行路径。
...
```bash
# 方式1: 使用 fdt_cli.py run 子命令（LangGraph 模式）
python fdt_cli.py run --mode default

# 方式2: HTTP API（fdt_api.py）
python fdt_api.py
...
| 状态管理 | 统一 DebateState |
| 调试 | LangSmith + 状态快照 |
| Checkpointer | PostgreSQL/SQLite 持久化 |
...
| 维度 | LangGraph 模式 |
|:-----|:--------------|
| 流程驱动 | StateGraph 内存传递 |
| 并行执行 | 声明式并行 |
| 状态管理 | DebateState + Checkpointer |
| 错误恢复 | 节点级错误捕获 + 状态回溯 |
| 监控 | 节点级计时 + 健康检查 |
