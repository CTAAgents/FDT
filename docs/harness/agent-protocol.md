...
│  └──┬──────────┬──────┘   └────▲──────────▲────┘                 │
│     │          │               │          │                      │
│     证据简报    辩论启动       判决输出(含交易参数) 风控结果      │
└─────┼──────────┼───────────────┼──────────┼──────────────────────┘
      │          │               │          │
      ▼          ▼               │          │
┌────────┐ ┌──────────┐ ┌──────────┐    │          │
│ 探源   │ │ 观澜     │ │ 读心     │    │          │
│ Funda- │ │ Technical│ │ Sentiment│    │          │
│ mental │ │ Output   │ │ StateVec │    │          │
│ State  │ │ (techni- │ │ (sentiment│   │          │
│ Vector │ │ cal.py)  │ │ _state.py)│   │          │
│ (funda- │ └──────────┘ └──────────┘    │          │
│ mental_ │                              │          │
│ state.  │                              │          │
│ py)    │                              │          │
...
| 角色 | Agent | P1 | P1.5 | P2 | P3 | P4 | P5 | P6 |
|:----|:------|:-:|:----:|:--:|:--:|:--:|:--:|:--:|
| 数技源 | datatech | ● 三类信号 | | | | | |
| 链证源 | chain-analyst | | ● 产业链 | | | | |
| **闫判官** | judge | | | **● 选品种+调度** | | **● 裁决(含交易参数)** | |
| 探源 | fundamental | | | | ● 供弹 | | |
| 观澜 | technical | | | | ● 供弹+支撑/阻力 | | |
| 读心 | sentiment | | | | ● 供弹（新闻情绪） | | |
| 多头分析员 | bullish_analyst | | | | ● 多头论据 | | |
| 空头分析员 | bearish_analyst | | | | ● 空头论据 | | |
| 风控明 | risk | | | | | ● 审核 | |
| **明鉴秋** | team-lead | **● 启动** | | | | **● 轮询+传递** | **● 归档+报告** | **● CTP信号输出** |
...
| 探源（fundamental-data-collector） | symbols list | 基本面因子数据采集 | `full_scan_factor_timing_*.json` | 无（原始数据JSON） | `fundamental-data-collector/scripts/reports/` |
...
| 读心 | 金十 MCP + WebSearch | 新闻情绪分析 | 情绪状态向量 | `SentimentStateVector` | `sentiment_state_*.json` |

### P3：辩论（闫判官主持）

| Agent | 输入 | 动作 | 输出 | Schema | 文件 |
|:------|:-----|:-----|:-----|:-------|:-----|
| 闫判官 | 数技源+链证源+探源+观澜+读心 | 汇总证据简报 → 选品种+调度 | `PrepBrief`（内部） | `PrepBrief` | 内存 |
...
| Agent | 输入 | 动作 | 输出 | Schema | 文件 |
|:------|:-----|:-----|:-----|:-------|:-----|
| 闫判官 | 多头+空头论据 | 裁决方向+交易参数 | `FinalJudgment`（含交易参数） | `FinalJudgment` | `judge_final_*.json` |
| 风控明 | FinalJudgment（含交易参数） | 5层风控审核 | `RiskOutput` | `RiskOutput` | `risk_*.json` |

### P5：归档（明鉴秋）

| Agent | 输入 | 动作 | 输出 | Schema | 文件 |
|:------|:-----|:-----|:-----|:-------|:-----|
| 明鉴秋 | FinalJudgment（含交易参数）+ RiskOutput + 所有中间产物 | 最终决策 | `TeamDecisionOutput` | `TeamDecisionOutput` | `debate_results.json` |
...
| Agent | 输入 | 动作 | 输出 | Schema | 文件 |
|:------|:-----|:-----|:-----|:-------|:-----|
| 明鉴秋 | TeamDecisionOutput | 生成CTP交易信号 | `SignalOutput` | `SignalOutput` | `ctp_signal_{trace_id}.json` |
...
| `SentimentStateVector` | `sentiment_state.py` | — | 1.0 🆕 | 读心 | 闫判官+多头+空头 |
...
| `SignalOutput` | `signal_output.py` | — | 1.0 🆕 | 明鉴秋 | CTP网关 |
...
| 读心 | ✅ | `SentimentStateVector` | `verdict`、多空结论 |
| 多头分析员 | ✅ | `ArgumentOutput(role="多头")` | — |
| 空头分析员 | ✅ | `ArgumentOutput(role="空头")` | — |
| 闫判官 | ✅ | `PrepBrief` (辩论前) / `FinalJudgment`(含交易参数) (判决) | — |
| 风控明 | ✅ | `RiskOutput` | `confidence > 0.9` |
| 明鉴秋 | ✅ | `TeamDecisionOutput` / `SignalOutput` | — |
...
p4_judge_{symbol}.json              # 闫判官终裁(含交易参数)
p4_coherence_{symbol}.json          # 一致性裁判
p5_risk_{symbol}.json               # 风控明
p6_signal_{trace_id}.json           # CTP信号输出
```
