...
version: 2.0
description: 商品期货产业链分析系统 — 产业链聚类+动态相关系数冗余检测(取代硬编码)+跨链品种主导链判断。链证源产出双报告供闫判官裁决。
agent_created: true
user_invocable: true
triggers:
  - 产业链分析
...
2. **产业链分析报告**: 动态相关系数(r>0.80标记冗余) + 产业链一致性验证 + Z分数极端性检查 + 基本面验证笔记
...
| 维度 | 规范 |
|:----|:-----|
| **来源** | 数技源 channel_breakout（scan_all.py → full_scan_summary_{date}.json）+ 探源 factor_timing（full_scan_factor_timing_{date}.json） |
| **文件** | `full_scan_factor_timing_{date}.json` + `full_scan_summary_{date}.json` |
...
┌─────────────────────┐    直接读取JSON      ┌──────────────────────┐
│  数技源 (P0-①)      │ ──────────────────→  │  链证源 (P0-②)       │
│  scan_all.py(channel_breakout)  │  价格/技术数据  │  analyze_chain.py  │
└─────────────────────┘                      │  + LLM链趋势解读      │
                                             │  + WebSearch基本面验证 │
┌─────────────────────┐    每条带URL+日期     └──────────────────────┘
│  WebSearch/WebFetch │ ──────────────────→          │
...
> ⚠️ **关键约束**：价格/技术数据来自数技源的已有产出，链证源不做二次采集。基本面数据来自互联网公开信息，每条必须有可追溯的来源。两个领域的数据**不可混用标签**——价格数据引用时注明`（来源：数技源通道突破）`，基本面数据引用时注明具体网站/报告名。
...
```
数技源(P0-①) → 链证源(P0-②) → 闫判官(P1) → 多空辩论(P2) → 风控明(P3)
```
...
## 🔒 Anti-Hallucination Circuit Breaker（链证源专用）
...
由 明鉴秋 传入辩论候选品种的结构化数据（含品种pid、方向）。
...
#### Step 5: 组合级产业链聚合 + 动态相关系数冗余检测

废弃硬编码相关系数表，改为 **60日滚动Pearson相关系数动态计算**。
...
3. 相关系数 r > threshold(默认0.80) → 标记为冗余配对
...
**搜索查询必须包含时间限定词**，如 `"纯碱 库存 2026年7月 最新 周度"`。

对每个候选品种所在产业链，搜索 2-3 条基本面信息：

```
查询1: "{产业链名} 供需 库存 最新" → 了解产业链整体供需格局
查询2: "{品种名} 新闻 政策 价格 最新" → 了解近期行业动态
查询3: "{品种名} 产能 产量 开工率 最新" → 了解供给侧情况

示例：对黑色系 rb/hc
查询1: "螺纹钢 热卷 库存 需求 房地产 基建 最新"
查询2: "钢铁 限产 政策 环保 粗钢产量 最新"
查询3: "铁矿石 焦炭 成本 利润 钢厂 最新"
```
...
当本 skill 被辩论专家团集成使用时，按以下 schema 结构化产出。
...
- `chain_results` 保持与现有逐品种JSON格式兼容
