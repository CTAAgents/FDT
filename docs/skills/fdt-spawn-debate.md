...
| 2 | **角色注入**：角色prompt在spawn prompt中手动注入，不依赖MD自动加载（v9.24.1 注：当前系统使用FdtAgentExecutor直调LLM，spawn约束已不再适用） | — |
| 3 | **禁止代写**：明鉴秋不得自行撰写论据、裁决、方案。必须spawn对应Agent | D01/D02 |
| 4 | **禁止串线**：Agent之间不得互相SendMessage。产出一律写文件 | S02 |
| 5 | **文件就绪**：spawn下游前，上游文件必须已稳定≥5秒（poll_file_ready） | S01/S04 |
| 6 | **禁止胶水**：不得编写一次性Python脚本模拟Agent产出 | 零胶水代码铁律 |
| 7 | **Phase门禁**：P6汇总前检查是否有缺失的产出文件。缺失则拒绝生成报告 | D03 |
| 8 | **D05降级**：闫判官spawn 2次均无产出 → 基于多头分析员/空头分析员论据独立裁决 | D05 |
| 9 | **🔴 文件优先通信**：Agent产出**只写文件，不使用SendMessage**。明鉴秋只用poll_file_ready轮询文件就绪后读取。SendMessage在自动化context中路由不可靠——Agent完成但消息永不送达 | A01 |
| 10 | **🔴 ADX角色反转**：ADX低位鼓励趋势启动确认、高位警示趋势过热。所有spawn prompt中必须注入ADX角色反转规则——不得以ADX<20作为"无趋势=不做"的首要理由。裁决reasoning中ADX提及占比≤1/3。监控条件不得以ADX为首要触发标准 | R11 |
| 11 | **🔴 L1产出校验**：每个Agent写文件→poll_file_ready就绪后，必须调用 `validate_agent_output.py --file <path> --phase <P4/P5_JUDGE/P5_RISK>` 做JSON解析+结构校验。校验失败（exit≠0，多为裸引号致JSON损坏）→ 立即重spawn该Agent（最多2次）。未校验不得进入下一阶段 | R29 |
| 12 | **🔴 Spawn 重试协议**：调用 Agent 工具 spawn 子Agent 时若返回工具错误（如 402 Insufficient Balance 等**瞬时**错误），立即用相同参数重试同款 spawn 最多2次（间隔5s），仍失败才进入降级（D05/明鉴秋直行）。禁止因单次瞬时错误放弃该阶段辩论 | R32 |
...
**当时修复**：统一使用`subagent_type: "general-purpose"` + 手动注入角色prompt。

**当前状态**（v9.24.1）：系统使用 FdtAgentExecutor 直调 LLM API，所有 Agent 节点为 LangGraph 图中的同步/异步函数调用，不再经过 TAE SDK spawn 机制。D05 铁律已移除。
