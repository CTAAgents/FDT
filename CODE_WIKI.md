...
| 自进化闭环 (Outer Loop) | Evolution Graph 驱动：APM 退化→self_improve 提案，样本≥5→校准权重，样本≥5→Agent 进化，样本≥50→ML 增量训练 |
| RHI 递归自进化 | **v9.21.0+** 新增，基于 pairwise 比较的 Harness 三层规范优化，与 evolution_graph 集成 |
...
| 文档一致性三层保障 | **v9.20.2 新增**：结构化元数据 + 自动校验脚本 + 数据驱动 YAML |
...
├──────────────────────────────────────────────────────────────┤
│                     编排层 (Orchestration)                    │
│  fdt_langgraph/ (3 个 LangGraph 子图 + RHI 子图)             │
│    ├── graph.py              — 辩论图结构 + 逐品种循环        │
│    ├── master_graph.py       — Master Orchestrator 图 (14 任务)│
│    ├── evolution_graph.py    — 自进化图 (APM-CS 五轴驱动)     │
│    ├── rhi_graph.py          — RHI 递归自进化子图 (v9.21.0)  │
│    ├── nodes.py              — 辩论节点函数 (P0-P6a)         │
│    ├── master_nodes.py       — Master 节点函数               │
│    ├── evolution_nodes.py    — 进化节点函数 (含 node_rhi)     │
...
├──────────────────────────────────────────────────────────────┤
│                     契约层 (Contracts)                        │
│  contracts/ — 统一数据信封、辩论 Schema、质检 Schema、RHI 规范 │
...
│    ├── rhi_harness_spec.py         — RHI 三层规范 (v9.21.0)  │
│    └── migrations.py               — 数据库迁移              │
├──────────────────────────────────────────────────────────────┤
│                     存储层 (Storage)                          │
...
│  scripts/   — 80+ 辅助脚本 (质检/进化/风控/CLI/RHI/运维)      │
...
FDT 通过四个独立的 LangGraph 子图分别管理不同职责，由 Master Graph 统一编排：
...
└────────────────────────────┬─────────────────────────────────────┘
                             ...
                             ▼
┌──────────────────────────────────────────────────────────────────┐
│  Debate Graph    (辩论图 — Inner Loop)                           │
│  scan → judge → per-symbol-loop → aggregate → report → signal   │
├──────────────────────────────────────────────────────────────────┤
│  Evolution Graph (自进化图 — Outer Loop)                         │
│  collect_metrics → apm_eval → decide_actions                     │
│    → [improve → calibrate → evolve → RHI → ml_train] → complete │
├──────────────────────────────────────────────────────────────────┤
│  RHI Graph       (递归自进化 — 与 Evolution 集成)                │
│  pairwise_eval → harness_optimize → apply_delta → verify         │
└──────────────────────────────────────────────────────────────────┘
```
...
FDT 包含 **4 个独立 LangGraph 子图**，复用 `state.py` / `agents.py` / `llm_provider.py` 等共享模块。
...
| RHI Graph | `rhi_graph.py` | `build_rhi_graph()` | 递归 Harness 自优化（v9.21.0+） |
...
| `rework_counters` | dict | {symbol: retry_count} 品种级重试 (v9.14.0) |
| `phase_timings` | list | 各阶段耗时记录 (v9.14.0) |
| `quality_metrics` | Optional[dict] | 自优化指标 (v9.14.0) |
...
| 单品种准备 | `node_prepare_one_symbol()` | P2.5 | 提取当前品种+准备 FDC |
...
| 质检 | `node_quality_inspect()` | P5.5 | 辩论输出质检 |
| 存储单品种 | `node_store_per_symbol_result()` | P5.75 | 存储品种结果 |
| 路由下一品种 | `node_route_next_symbol()` | P5.8 | 路由下一品种 |
| 汇聚结果 | `node_aggregate_results()` | P5.9 | 全部品种汇聚 |
...
**进化图执行流程** (v9.22.0 含 RHI)：

```
collect_metrics → apm_eval → decide_actions
    → [improve → calibrate → evolve → RHI → ml_train → ...] → complete → END
```

各步骤条件路由：
- `route_after_decide`: improve → calibrate → evolve → rhi → ml → complete
- `route_after_improve`: calibrate → evolve → rhi → ml → complete
- `route_after_calibrate`: evolve → rhi → ml → complete
- `route_after_evolve`: rhi → ml → complete
- `route_after_rhi`: ml → complete
...
| 决策 | `node_decide_actions` | 基于阈值+样本量决策，含 `need_rhi` |
...
| **RHI 自优化** | **`node_rhi`** | **v9.22.0 新增 — `scripts/rhi_harness_optimizer.py` 驱动** |
...
| `_apply_decode_config()` | D3 Generation 解码控制 |
...
#### 3.1.14 rhi_graph.py — RHI 递归自进化子图 (v9.21.0+)

RHI (Recursive Harness Self-Improvement) 基于 pairwise 比较优化 Harness 三层规范。

**配置常量**:

| 常量 | 默认值 | 说明 |
|:------|:------|:------|
| `RHI_MAX_ITER` | 5 | 最大迭代次数 |
| `RHI_EPSILON` | 0.3 | 收敛阈值（改进率低于此值停止） |

**核心节点函数**:

| 函数 | 说明 |
|:------|:------|
| `_build_base_harness()` | 从 agents/*.yaml / graph.py / harness-rules.yaml 构建初始 Harness |
| `node_rhi_iteration(state)` | 单次 RHI 迭代：pairwise_eval → optimize → apply |
| `_check_convergence(state)` | 检查改进率是否低于 ε |

**RHI 子图流程**:

```
build_base_harness → rhi_iteration → check_convergence
    → (未收敛 + 未超 max_iter) → rhi_iteration
    → (收敛或超限) → complete
```

#### 3.1.15 evolution_state.py — 自进化状态 (v9.17.0+)

**EvolutionState** (dict subclass) — 自进化闭环状态：

| 字段 | 类型 | 说明 |
|:------|:------|:------|
| `trace_id` | str | 全链路追踪 ID |
| `phase` | str | 当前阶段 (idle/collecting/apm_eval/deciding/improving/calibrating/evolving/rhi/ml_training/completed) |
| `source_trace_id` | str | 触发进化的辩论 trace_id |
| `collected_metrics` | dict | 品藻/D3/D6 质量指标 |
| `apm_scores` | dict | D1-D5 五轴评分 |
| `apm_overall` | dict | 综合评分与退化标记 |
| `decisions` | dict | need_improve/calibrate/evolve/rhi/ml_train |
| `step_results` | dict | 各步骤执行结果 |
| `errors` | list | 错误列表 |
...
#### 3.3.4 rhi_harness_spec.py — RHI 三层规范 (v9.21.0+)

定义 RHI (Recursive Harness Self-Improvement) 的三层规范：

| TypedDict | 说明 |
|:----------|:------|
| `HarnessSpec` | 三层 Harness 表示（agent_candidates + workflow + auxiliary_rules） |
| `AgentContract` | 子 Agent 向编排器的输出契约（role/instruction/contract_fields/llm_config） |
| `WorkflowHop` | 工作流中的单步交互（name/agents/input_from/output_to/timeout/fallback） |
| `Workflow` | 工作流定义（contracts + hops + orchestrator_instruction） |
| `AuxiliaryRules` | 辅助规则（验收门禁/回退/通信/召回） |
| `PairwisePreference` | 两轮产出的 pair-wise 对比结果 |
| `RHIHistory` | RHI 迭代历史（含改进率） |

参考论文: RHI (arXiv:2607.15524), MemoHarness (arXiv:2607.14159)

#### 3.3.5 experience_schema.py — 经验 Schema

定义经验记录的结构化格式（配方+评分），用于自进化闭环存储与复用。

#### 3.3.6 migrations.py — 数据库迁移
...
| `webui.py` | Web 管理界面 |
| `verify_doc_consistency.py` | 文档一致性自动校验 (v9.20.2) |
| `rhi_global_cli.py` | RHI 全局 CLI 工具 (v9.22.0) |
| `rhi_global_harness.py` | 全局 Harness 自优化 (v9.21.0) |
| `rhi_harness_optimizer.py` | RHI Harness 优化器 (v9.21.0) |
| `rhi_pairwise_eval.py` | pairwise 对比评估 (v9.21.0) |
| `rhi_global_setup.py` | RHI 全局初始化 (v9.22.0) |
...
| `HarnessSpec` | `contracts.rhi_harness_spec` | RHI 三层规范 (v9.21.0) |
| `PairwisePreference` | `contracts.rhi_harness_spec` | pairwise 对比结果 (v9.21.0) |
...
| `build_rhi_graph(base_harness)` | `fdt_langgraph.rhi_graph` | 构建 RHI 自优化子图 (v9.21.0) |
| `evaluate_pairwise(before, after)` | `scripts.rhi_pairwise_eval` | pairwise 四维对比评分 (v9.21.0) |
| `compute_improvement_rate(history)` | `scripts.rhi_pairwise_eval` | 改进率计算 (v9.21.0) |
...
| `node_prepare_one_symbol(state)` | `fdt_langgraph.nodes` | P2.5 单品种准备 |
...
| `node_quality_inspect(state)` | `fdt_langgraph.nodes` | P5.5 质检 |
| `node_report(state)` | `fdt_langgraph.nodes` | P6 明鉴秋报告 |
| `node_rhi(state)` | `fdt_langgraph.evolution_nodes` | RHI 自优化节点 (v9.22.0) |
...
fdt_langgraph (graph → nodes → agents → state → health → quality_inspector → llm_provider)
    │   ├── master_graph / evolution_graph / rhi_graph (独立子图)
    ↓
data_source_adapter
    ↓
futures_data_core (multi_source_adapter → collectors → f10 → indicators → mcp_client)
    ↓
contracts (a2a_payload → debate_argument_schema → debate_quality_schema → rhi_harness_spec)
    │
fdt_pg / fdt_cache / memory
```
...
- `scripts.rhi_*` 脚本依赖 `contracts.rhi_harness_spec` 规范
...
```bash
# 单次辩论执行
python fdt_cli.py run [--mode default/fast/deep_research/tournament] [--evolve]
...
# 自进化独立运行
python fdt_cli.py evolve [--trace-id <id>]
...
├── contracts/                   # 契约定义（A2A/辩论 Schema/质检 Schema/RHI 规范）
├── debate/                      # 辩论历史管理
├── docs/                        # 文档体系
...
├── memory/                      # 记忆系统测试
├── pipeline/                    # 流水线测试
├── scheduler/                   # 调度引擎测试（退役中）
├── validators/                  # 验证器测试
├── self-improve-enhanced/       # 自改进增强测试
├── loop_engine/                 # 循环引擎测试
├── rhi/                         # RHI 自优化测试（22+ 用例，v9.21.0+）
└── conftest.py                  # 全局测试配置
```

合计：17 个测试子目录 / 1400+ 用例
...
### 8.2 文档一致性三层保障（v9.20.2+）

| 层 | 机制 | 内容 |
|:--:|:------|:------|
| Layer 1 | 结构化元数据 | 每篇 `docs/harness/*.md` 末尾必须有 `## 一致性元数据` 表格（代码→文档映射+可验证断言+检验方式） |
| Layer 2 | 自动校验脚本 | `scripts/verify_doc_consistency.py` 自动校验 — commit 前或 `docs/harness/` 变更后运行 |
| Layer 3 | 数据驱动 | 易变配置存入 `docs/harness/_data/*.yaml`，文档通过引用保持一致 |
...
| 13 | 文档一致性检查 | 运行 `python scripts/verify_doc_consistency.py` |
...
## 9. RHI 递归自进化体系（v9.21.0+）

### 9.1 体系概述

RHI (Recursive Harness Self-Improvement) 基于 **pairwise 对比** 优化 Harness 三层规范，参考论文：
- RHI: arXiv:2607.15524 — Recursive Harness Self-Improvement
- MemoHarness: arXiv:2607.14159 — Agent Harnesses That Learn from Experience

### 9.2 三层 Harness 规范

| 层 | 类型 | 说明 |
|:--:|:------|:------|
| Layer 1 | Agent Candidates | 子 Agent 输出契约（role/instruction/contract_fields/llm_config） |
| Layer 2 | Workflow (Contract + Hop) | 工作流步骤定义（agents/input_from/output_to/timeout/fallback） |
| Layer 3 | Auxiliary Rules | 辅助规则（验收门禁/回退/通信/召回） |
...
│                                                                  │
│  ┌──────────────┐    ┌──────────────────┐    ┌───────────────┐  │
│  │ pairwise_eval │───→│ harness_optimizer │───→│ apply_delta  │  │
│  │ (四维评分)     │    │ (LLM 生成方案)     │    │ (修改配置文件) │  │
│  └──────┬───────┘    └──────────────────┘    └───────┬───────┘  │
│         │                                             │          │
│         └─────── 收敛判断 (ε=0.3) ────────────────────┘          │
│                    ↓ 收敛或 max_iter=5                             │
...
| 维度 | 权重 | 说明 |
|:----:|:----:|:------|
| 质检通过率 | 0.30 | validate_argument/verdict/risk 的通过比例 |
| 风控准确率 | 0.25 | 风控判断与后续验证一致性 |
| 信号命中率 | 0.25 | CTP 信号的 T+1 验证结果 |
| 报告完整性 | 0.20 | HTML 报告的字段完整度 |

**改进率计算**：`improvement_rate = (sum(scores_after) - sum(scores_before)) / sum(scores_before)`

**停止条件**：`improvement_rate < ε (0.3)` 或 `max_iter (5)`

### 9.5 RHI 组件速查

| 组件 | 路径 | 说明 |
|:------|:------|:------|
| 三层规范 | `contracts/rhi_harness_spec.py` | HarnessSpec / AgentContract / WorkflowHop / PairwisePreference / RHIHistory |
| Pairwise 评估 | `scripts/rhi_pairwise_eval.py` | `evaluate_pairwise()` 四维评分 + `compute_improvement_rate()` |
| Harness 优化器 | `scripts/rhi_harness_optimizer.py` | `build_optimizer_prompt()` + `parse_optimizer_response()` + `apply_config_delta()` |
| RHI LangGraph 子图 | `fdt_langgraph/rhi_graph.py` | 独立 RHI 子图（build_rhi_graph） |
| evolution 集成 | `fdt_langgraph/evolution_nodes.py` | `node_rhi()` — 在进化闭环中自动触发 RHI |
| 全局 Harness RHI | `scripts/rhi_global_harness.py` | CLAUDE.md 自优化 |
| 全局 CLI | `scripts/rhi_global_cli.py` | 5 子命令：init / status / step / history / install |

### 9.6 RHI 全局 CLI

任何项目均可使用 RHI 全局 Harness 自优化：

```bash
# 初始化 RHI（检测项目类型，创建 harness 目录）
python scripts/rhi_global_cli.py init
...
# 查看历史
python scripts/rhi_global_cli.py history

# 安装 pre-commit hook
python scripts/rhi_global_cli.py install
```

环境变量：`FDT_RHI=true` 启用 evolution_graph 中的 RHI 分支
...
| **v9.20.0** | **v9.19.0 生产运行问题修复批** — Evolution NoneType / 质检缺字段容错 / LLM 解析回退 / FDC N/A 兜底 |
| **v9.19.0** | **LangGraph 迁移收尾 · G108 关闭** — pipeline/runner.py 退役 + A/B 切换清理 + 心跳文件落地 |
| **v9.18.0** | **Master Orchestrator Graph** — 全量自动化迁移至 LangGraph，14 任务统一编排 |
| **v9.17.0** | **自进化闭环 Evolution Graph** — APM-CS 五轴驱动的外循环 LangGraph 子图 |
| **v9.16.0** | **D2/D5/D6 工程成熟度提升** — OutputMetrics + ToolMetrics + memory_cleaner |
| **v9.15.0** | **Generation Phase 4** — 解码参数反馈闭环（升温重试） |
| **v9.14.0** | **Phase 3 Data Governance** — 辩论输出质检 + 不合格退回重修（最多 2 次） |
| **v9.13.0** | **逐品种独立辩论循环** + 品藻角色拆分 |
| **v9.11.0** | **读心 Agent 落地** — 新闻情绪分析因子，P3 三源→四源并行 |
