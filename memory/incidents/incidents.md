...
## 2026-07-24 | 辩论报告缺少 P3.5 品藻质检 + 交易建议使用挂单价

**事件**: CF2609/PTA609 辩论报告生成时：
1. 缺少 P3.5 品藻质检可视化环节
2. PTA609 交易建议给出"5,850-5,950 回调区间"挂单价
3. P6 裁决汇总展示原始 JSON 源码
4. RSI/ADX 来自网络数据未标注，与通达信有偏差

**根因**:
1. 品藻质检仅在后台运行未可视化输出
2. 未遵循"市价入场"原则，错误使用了等待回调策略
3. 直接输出了规范化 JSON 数据而非渲染为 UI
4. 使用了金投网/曲合期货数据未做来源标注

**改正**:
1. 新增 P3.5 品藻质检可视化章节（含校验项逐条结果）
2. 入场价改为当前市价 6,020，止损 5,800，盈亏比 1.27:1
3. JSON 源码改为中文可视化交易参数总表（10 字段）
4. 加入技术指标数据源说明框 + FDC 完整计算公式

**预防**:
1. P0 铁律：交易建议必须使用市价（已记入 MEMORY.md）
2. 辩论报告必须包含 P3.5 可视化质检章节
3. 报告禁止展示原始 JSON 源码
4. 技术指标必须标注数据来源
