...
## 右侧交易建议（2026-07-26 掌柜确立）

反趋势方向且趋势结构未被破坏时，建议优先等待结构确认，统一标记为 INFO（仅供关注），不建议给出可执行参数。基本面决定方向对不对，技术面决定什么时候入场。此条为经验建议，非硬性规则，具体执行由判官根据实际走势自行判断。

## 🔴 P0 执行规则：辩论流程触发语义映射（2026-07-27 掌柜确立）

当用户说以下短语之一，**必须执行 FDT 真实辩论代码**，禁止手动模拟：
- "走辩论流程" / "走深度研究辩论流程" / "跑辩论" / "跑FDT"
- "用FDT分析" + 品种名称
- "辩论分析" + 品种名称

### 执行前的强制环境检查顺序
1. [环境] python -c "from fdt_langgraph import *" 是否可导入
2. [数据] 通达信TQ-Local / AKShare 是否可获取实时行情
3. [数据库] PostgreSQL (pg) 是否可连接
4. [密钥] FDT_LLM_API_KEY 环境变量是否有值
5. [决策] 以上任一缺失 → 打印缺失清单并询问用户是否修复，不得跳过
